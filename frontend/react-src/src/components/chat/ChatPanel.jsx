...
export default function ChatPanel() {
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, x: 20 }}
      className="fixed right-4 bottom-4 w-80 h-[480px] bg-slate-900 border border-slate-700 rounded-xl shadow-2xl flex flex-col z-50 overflow-hidden"
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 bg-slate-900">
        ...
        <button onClick={() => dispatch(toggleChat())} className="text-slate-500 hover:text-slate-200">
          <X className="w-4 h-4" />
        </button>
      </div>
        ...
        <div className="flex gap-1 px-3 py-2 border-b border-slate-800 overflow-x-auto">
          {channels.map((ch) => (
            <button
              ...
              className={`px-2 py-1 rounded text-xs shrink-0 transition-colors ${activeChannelId === ch.id ? 'bg-emerald-500/20 text-emerald-400' : 'text-slate-400 hover:text-slate-200'}`}
            >
              #{ch.name}
            </button>
          ))}
            ...
            return (
              <div key={msg.id} className={`flex gap-2 ${isMe ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[75%] ${isMe ? 'items-end' : 'items-start'} flex flex-col gap-0.5`}>
                  ...
                  <div className={`px-3 py-2 rounded-xl text-sm ${isMe ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-200'}`}>
                    {msg.content}
                  </div>
      ...
      <div className="px-3 py-2 border-t border-slate-800 flex gap-2">
        <input
          ...
          placeholder="Message…"
          className="flex-1 bg-slate-800 border border-slate-700 rounded-lg px-3 py-1.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-emerald-500"
        />
        <button
          onClick={handleSend}
          disabled={!text.trim()}
          className="p-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 rounded-lg text-white transition-colors"
        >
